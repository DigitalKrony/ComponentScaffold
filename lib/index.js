    ...
    switch (type) {
        ...
        case 'camel':
            let toReturn = string.replace(/^([A-Z]|[a-z])|[\s\-\_](\w)/g, function(data) {
                return data.replace(/[(\-|\_)]/g, '').trim().toUpperCase();
            });
...
class ComponentScaffold {
    constructor (args) {
        this.config = fs.existsSync('./scaffold.config.json') ? JSON.parse(fs.readFileSync('./scaffold.config.json')) : JSON.parse(fs.readFileSync(`${__dirname}/scaffold.config.json`));
        ...
        for(var i = 0; i < json.length; i++) {
            ...
            switch(thisObj.type) {
                case 'file':
                    ...
                    fs.writeFile((`${root}/${nameConcat}.${thisObj.extension}`).trim(),  this.replaceTokens(thisObj.content), (err) => { if (err) { console.log(err);} });
    ...
    startTask () {
        ...
        if (!fs.existsSync(fileDirectory)) {
            ...
            console.log(`Your new ${this.options.group}, ${this.options.name.pascal}, has been created.`);
        } else {
            console.log(`The new ${this.options.group.toUpperCase()}, ${this.options.name.pascal}, you are trying to create is not UNIQUE within ${this.options.group.toUpperCase()}.`);
        }
    }
}
